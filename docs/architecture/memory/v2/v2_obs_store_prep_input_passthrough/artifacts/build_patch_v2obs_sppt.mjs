#!/usr/bin/env node
// Deterministic builder for V2-OBS-STORE-PREP-INPUT-PASSTHROUGH.
// Modifies: ME_Memory_Store_Prep.parameters.jsCode,
//           ME_Memory_Store_DB.parameters.query,
//           ME_Memory_Store_DB.parameters.options.queryReplacement.
...
const prePath  = path.join(here, 'WF-ME-01_pre_v2obs_sppt.json');
const postPath = path.join(here, 'WF-ME-01_post_v2obs_sppt.json');
const diffPath = path.join(here, 'diff_summary.md');
const pre = JSON.parse(fs.readFileSync(prePath, 'utf8'));

// ---- new Store_Prep jsCode ----
// Derived from live Store_Prep jsCode by adding tier / user_confirmed /
// corroboration_count extraction with safe defaults.
...
const newStoreDBQueryReplacement = "={{ $json._error ? [null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null] : [$json.__db.tenant_id, $json.__db.memory_type, $json.__db.category, $json.__db.content, $json.__db.confidence, $json.__db.importance, $json.__db.durability, $json.__db.source_thread_id, $json.__db.source_message_id, $json.__db.entity_id, $json.__db.evidence_refs, $json.__db.metadata, $json.__db.idempotency_key, $json.__db.tier, $json.__db.user_confirmed, $json.__db.corroboration_count, $json.__db.embedding_text] }}";

// ---- Compose post ----
const post = JSON.parse(JSON.stringify(pre));
const sp = post.nodes.find(n => n.name === 'ME_Memory_Store_Prep');
if (!sp) throw new Error('BUILD-INV-2 violated: Store_Prep not found');
const sdb = post.nodes.find(n => n.name === 'ME_Memory_Store_DB');
if (!sdb) throw new Error('BUILD-INV-2 violated: Store_DB not found');
...
if (binds.size !== 17) throw new Error('BUILD-INV-5 violated: expected 17 bind slots, got ' + binds.size);
if (!/CASE\s+WHEN\s+\$17::text\s+IS\s+NULL\s+THEN\s+NULL\s+ELSE\s+\$17::vector\(1536\)\s+END/i.test(sdb.parameters.query)) {
  throw new Error('BUILD-INV-5 violated: $17 CASE guard missing');
}

const qr = sdb.parameters.options.queryReplacement;
const succ = qr.match(/:\s*\[([^\]]*embedding_text[^\]]*)\]/);
if (!succ) throw new Error('BUILD-INV-6 violated: success branch missing embedding_text');
const refs = (succ[1].match(/\$json\.__db\./g) || []).length;
if (refs !== 17) throw new Error('BUILD-INV-6 violated: success refs != 17: ' + refs);
const err = qr.match(/\?\s*\[([^\]]*)\]/);
if (!err) throw new Error('BUILD-INV-6 violated: error branch missing');
const nulls = (err[1].match(/\bnull\b/g) || []).length;
if (nulls !== 17) throw new Error('BUILD-INV-6 violated: error branch nulls != 17: ' + nulls);

if (!/const VALID_TIERS = \['recent','long_term'\];/.test(sp.parameters.jsCode)) throw new Error('BUILD-INV-7 violated: tier validation missing');
if (!/const tier = .+VALID_TIERS\.includes\(inputs\.tier\).+'recent'/.test(sp.parameters.jsCode)) throw new Error('BUILD-INV-7 violated: tier fallback missing');
if (!/inputs\.user_confirmed === true \|\| inputs\.user_confirmed === false/.test(sp.parameters.jsCode)) throw new Error('BUILD-INV-7 violated: user_confirmed strict check missing');
if (!/Number\.isInteger\(inputs\.corroboration_count\) && inputs\.corroboration_count >= 1/.test(sp.parameters.jsCode)) throw new Error('BUILD-INV-7 violated: corroboration_count >=1 validation missing (DB CHECK requires >=1)');
...
const diff = `# V2-OBS-STORE-PREP-INPUT-PASSTHROUGH — Diff Summary
