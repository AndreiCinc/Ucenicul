...
|---|---|---|
| `task_module` | Creates, updates, lists, closes operational tasks. **Current-stage canonical owner of reminder-like requests** (see `decisions/ADR-REMINDER-AS-TASK-LAYER.md`): "remind me to ..." maps to `create_task` with `due_at`/`due_date`. | MVP required |
| `reminder_module` | (Long-term) creates, updates, lists, triggers reminders. **Current-stage status: deferred** -- reframed as future `REMINDER-DELIVERY-LAYER` (scheduler + temporal trigger + MO delivery + sent/snooze/retry/audit). No CRUD path through the canonical chain in the current stage. See `decisions/ADR-REMINDER-AS-TASK-LAYER.md`. | Deferred (was MVP-framed) |
