...
## SQL binding policy (per `docs/n8n_Workflow_Mapping.md` Section 5)
- Canonical SQL under `workflows/sql/or/` uses parameterized binding (`$1`, `$2`, `$3`) and is the source of truth for live import.
- The live `OR_Load_Execution_Context` Postgres node uses sanitized inline interpolation (`'{{ $json.execution_id }}'::uuid` and equivalents) because the n8n Postgres node v2.5 `executeQuery` mode does not cleanly support parameterized binding for a templated query string. All interpolated values originate from `OR_Extract_Handoff_Input` after `OR_Validate_EC_Result` has enforced UUID shape and required-field checks, and are immediately cast with `::uuid` to fail closed on any non-UUID input.
- This deviation is recorded here as the documented "why parameterized binding was not used" required by the workflow-mapping policy.
