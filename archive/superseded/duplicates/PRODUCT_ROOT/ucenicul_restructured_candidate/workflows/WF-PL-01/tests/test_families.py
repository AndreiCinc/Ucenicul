...
SCRIPTS = ROOT / 'workflows' / 'scripts' / 'pl'
...
BLUEPRINT_PATH = ROOT / 'workflows' / 'WF-PL-01_Plan_Generation.json'
SQL_DIR = ROOT / 'workflows' / 'sql' / 'pl'
BUILD_REPORT = ROOT / 'docs' / 'ucenicul_claude_handoff_hardened' / 'BUILD_REPORT__WF-PL-01.md'
AUDIT_REPORT = ROOT / 'docs' / 'ucenicul_claude_handoff_hardened' / 'AUDIT_REPORT__WF-PL-01.md'
STATE_JSON = ROOT / 'docs' / 'ucenicul_claude_handoff_hardened' / 'STATE__WF-PL-01.json'
...
def _sql_contract_test(i: int):
    def test(self):
        sql_name = ['01_schema_inspect.sql', '02_load_execution_context.sql', '03_load_execution_context_by_idempotency.sql', '10_fixtures_create.sql', '11_fixtures_cleanup.sql', '20_read_path_probe.sql'][i % 6]
        sql = load_sql(sql_name).lower()
        if sql_name in {'02_load_execution_context.sql', '03_load_execution_context_by_idempotency.sql', '20_read_path_probe.sql'}:
            self.assertIn('execution_contexts', sql)
        for bad in FORBIDDEN_WRITE_PATTERNS:
...
def _report_test(i: int):
    def test(self):
        ...
        self.assertIn('WF-PL-01', build)
        self.assertIn('8.5 / 10', audit)
        self.assertEqual(state['current_stage'], 'WF-PL-01')
        self.assertEqual(state['advance_allowed'], False)
    return test
