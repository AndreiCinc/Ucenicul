...
def family_reporting_and_tooling_contract():
    stage_path = ROOT.parent / 'docs' / 'ucenicul_claude_handoff_hardened' / '08_STAGE_WF-DI-01.md'
    audit_path = ROOT.parent / 'docs' / 'ucenicul_claude_handoff_hardened' / 'AUDIT_REPORT__WF-DI-01.md'
    closure_path = ROOT.parent / 'docs' / 'ucenicul_claude_handoff_hardened' / 'CLOSURE_REPORT__WF-DI-01.md'
    for i in range(50):
        assert_true(stage_path.exists(), 'stage file missing')
        assert_true(audit_path.exists(), 'audit report missing')
        assert_true(closure_path.exists(), 'closure report missing')
        closure_text = closure_path.read_text(encoding='utf-8')
        assert_true('NOT CLOSED' in closure_text, 'closure report must stay honest pre-live')
...
def family_blueprint_structure():
    workflow_path = ROOT / 'WF-DI-01_Dispatcher.json'
    for i in range(50):
